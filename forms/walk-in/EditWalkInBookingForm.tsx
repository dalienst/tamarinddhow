...
interface EditWalkInBookingFormProps {
  token: string;
  onSuccess: () => void;
...
export default function EditWalkInBookingForm({ token, onSuccess, bookingToEdit, initialPayment }: EditWalkInBookingFormProps) {
  ...
  // Parse initial payment info (to avoid useEffect)
  const initialTotal = parseFloat(bookingToEdit.total_amount.toString());
  const initialPaid = initialPayment ? parseFloat(initialPayment.amount) : 0;
  
  const isInitiallyPartial = initialPayment && (initialPaid < initialTotal);

  // Initialize Form State directly from bookingToEdit
  const [selectedScheduleId, setSelectedScheduleId] = useState(bookingToEdit.schedule || "");
  const [guestName, setGuestName] = useState(bookingToEdit.primary_guest_name || bookingToEdit.booked_by_name || "");
  const [guestEmail, setGuestEmail] = useState(bookingToEdit.primary_guest_email || "");
  const [guestPhone, setGuestPhone] = useState(bookingToEdit.primary_guest_phone || "");
  const [adultCount, setAdultCount] = useState(bookingToEdit.adult_count?.toString() || "2");
  ...
  const [paymentState, setPaymentState] = useState<"unpaid" | "cash" | "mpesa" | "agent_credit" | "waived" | "staff_card" | "mastercard" | "visa">(
    initialPayment ? initialPayment.payment_method : "unpaid"
  );
  const [transactionRef, setTransactionRef] = useState(initialPayment?.transaction_ref || "");
  
  // Custom prices
  const [customAdultPrice, setCustomAdultPrice] = useState(bookingToEdit.custom_price_per_person?.toString() || "");
  const [customChildPrice, setCustomChildPrice] = useState(bookingToEdit.custom_price_per_child?.toString() || "");

  // Discount options
  const [discountType, setDiscountType] = useState<"amount" | "percentage">(bookingToEdit.discount_type || "amount");
  const [discountValue, setDiscountValue] = useState(bookingToEdit.discount_value?.toString() || "0");
  const [discountReason, setDiscountReason] = useState(bookingToEdit.discount_reason || "");

  // Selected addons
  const initialAddons: SelectedAddonItem[] = bookingToEdit.booking_addons 
    ...
    : [];
  const [selectedAddons, setSelectedAddons] = useState<SelectedAddonItem[]>(initialAddons);

  const [isPartialPayment, setIsPartialPayment] = useState(!!isInitiallyPartial);
  const [partialPaymentType, setPartialPaymentType] = useState<"amount" | "percentage">("amount");
  const [partialPaidAmount, setPartialPaidAmount] = useState(isInitiallyPartial ? initialPaid.toString() : "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (paymentState === "unpaid") {
        if (initialPayment) {
          await axios.delete(`/api/v1/payments/${initialPayment.reference}/`, {
            headers: { Authorization: `Token ${token}` }
          });
        }
        ...
        if (initialPayment) {
          await axios.put(
            `/api/v1/payments/${initialPayment.reference}/`,
            {
      ...
      console.error("Booking update error:", err.response?.data);
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6 animate-fadeIn relative">
        {/* Delete Button (Absolute positioned top right if booking is active) */}
        {bookingToEdit.check_in_status !== "checked_in" && (
          <button
            type="button"
            onClick={() => setShowDeleteModal(true)}
            className="absolute top-6 right-6 p-2 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-lg transition-colors border border-rose-100 flex items-center gap-2 text-sm font-bold"
        ...
        <div className="pt-2">
          ...
          <select
            ...
            className="w-full sm:w-2/3 px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm bg-white focus:ring-2 focus:ring-amber-500/20 disabled:opacity-60 font-semibold text-slate-800"
          >
            {loadingSchedules ? (
            ...
            {/* If the current schedule is past, we should still show it as an option if they don't change it */}
            {!upcomingSchedules.find(s => s.id === selectedScheduleId) && selectedSchedule && (
              <option value={selectedScheduleId}>
                {selectedSchedule.dhow_name} - {selectedSchedule.date} ({selectedSchedule.meal_type_display}) [Past]
              </option>
            )}
          </select>
        </div>
        ...
        <button
          type="submit"
          disabled={isSaving}
          className="w-full py-3 bg-amber-600 hover:bg-amber-700 disabled:bg-amber-700/60 text-white font-bold text-base rounded-xl transition-all shadow-md shadow-amber-600/10 flex items-center justify-center gap-2 disabled:cursor-not-allowed mt-4"
        >
          {isSaving ? (
            <>
